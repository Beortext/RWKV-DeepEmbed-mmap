...
class DeepEmbed_Loader(nn.Module):
    __constants__ = ['data_file']

    def __init__(self, data_file:str, dtype=DTYPE):
        super().__init__()
        self.data_file = data_file
        self.dtype = dtype

    def forward(self, key_name:str, tokens:torch.Tensor) -> torch.Tensor:
        original_device = tokens.device
        tokens = tokens.cpu()
        output = torch.ops.custom.perform_io_task_sync(self.data_file, key_name, tokens)
        return output.to(device=original_device, dtype=self.dtype)

########################################################################################################

class RWKV_x070(MyModule):
    def __init__(self, model_name, de_file = "DeepEmbed.bin"):
        super().__init__()
        self.eval()
        ...
        with torch.no_grad(): 
            ...
            for i in range(self.n_layer):
                ...
                qkv, state[self.n_layer*3+1+24+i], state[self.n_layer*3+1+i*2], state[self.n_layer*3+1+i*2+1] = RWKV_v7s_DEA(
                    ...
                    z[qkv+'k1'], z[qkv+'k2'], state[self.n_layer*3+1+i*2], self.de_loader(f"k_emb.{i}", ctx),
                    z[qkv+'v1'], z[qkv+'v2'], state[self.n_layer*3+1+i*2+1], self.de_loader(f"v_emb.{i}", ctx),
                ...
                xx, state[i*3+2] = RWKV_x070_CMix_one(xx, state[i*3+2], z[ffn+'x_k'], z[ffn+'key.weight'], z[ffn+'value.weight'],
                     self.de_loader(f"s_emb.{i}", ctx[-1:]),
        ...
        with torch.no_grad(): 
            ...
            for i in range(self.n_layer):
                ...
                xx, state[i*3+2] = RWKV_x070_CMix_seq(xx, state[i*3+2], z[ffn+'x_k'], z[ffn+'key.weight'], z[ffn+'value.weight'],
                     self.de_loader(f"s_emb.{i}", ctx[-len(idx):]),
                     z[ffn+'s1'], z[ffn+'s2'], z[ffn+'s0'])
                x = x + xx
...
def RWKV_v7s_DEA(x, Q_, q_prev, k1, k2, k_c, k_e, v1, v2, v_c, v_e, x_q, x_k, x_v, lnq_w, lnq_b, lnk_w, lnk_b, lnv_w, lnv_b):
    ...
    if is_seq_mode:
        ...
        mask = ~torch.tril(torch.ones(ctx_len, ctx_len, dtype=torch.bool, device=scores.device))[-T:, :]
